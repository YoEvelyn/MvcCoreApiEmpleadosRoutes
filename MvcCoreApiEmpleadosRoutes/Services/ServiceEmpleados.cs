...
namespace MvcCoreApiEmpleadosRoutes.Services
{
    public class ServiceEmpleados
    {
        ...
        //METODO INTERMEDIO PARA LEER LOS DATOS DEL API INDEPENDIENTEMENTE
        //DE LO QUE LEAMOS
        ...
        public async Task<Empleado> FindEmpleadoAsync(int id)
        {
            ...
            Empleado empleaddo = await this.GetDatosApiAsync<Empleado>(request);
            return empleaddo;
        }

        public async Task<List<Empleado>> GetEmpleadosOficiosAsync(string oficio)
        {
            string request = "/api/empleados/findempleadosoficio/" + oficio;
            List<Empleado> empleados = await this.GetDatosApiAsync<List<Empleado>>(request);
            return empleados;
        }
